...
* 从上面的汇编和C语言代码的执行流程分析来看，汇编的执行和C代码执行的顺序不是完全一行一行翻译的，而是经过了优化处理的，提高了代码的执行的效率，可以通过`barrier()`函数进行防止乱序优化；
* `volatile`表示某个变量容易改变，不要直接拿`r0~r7`(`x0~x7`)中的值，要读取实际内存的值，未添加的时候，同一个函数多次访问同意变量，汇编的时候可能就被优化了，只读一次；
