...
static bool check_mac_perms(pid_t spid, const char *tctx, const char *perm, const char *name)
{
    ...
    //     printf("SELinux: getpidcon(pid=%d) failed to retrieve pid context.\n", spid);
    //     return false;
    // }
...
static bool check_mac_perms_from_lookup(pid_t spid, const char *perm, const char *name)
{
    ...
    // if (!sehandle) {
    //     printf("SELinux: Failed to find sehandle. Aborting service_manager.\n");
    //     abort();
    // }

    // if (selabel_lookup(sehandle, &tctx, name, 0) != 0) {
    //     printf("SELinux: No match for %s in service_contexts.\n", name);
    //     return false;
    // }
                   ...
                   pid_t spid)
{
    ...
    if (!svc_can_register(s, len, spid)) {
        printf("add_service('%s',%x) uid=%d - PERMISSION DENIED\n",
                   ...
                   struct binder_io *reply)
{
    ...
    case SVC_MGR_LIST_SERVICES: {
        ...
        if (!svc_can_list(txn->sender_pid)) {
            printf("list_service() uid=%d - PERMISSION DENIED\n",
                    txn->sender_euid);
            return -1;
        }
        si = svclist;
...
int main(int argc, char **argv)
{
    ...
    // if (selinux_enabled > 0) {
    //     if (sehandle == NULL) {
    //         printf("SELinux: Failed to acquire sehandle. Aborting.\n");
    //         abort();
    //     }

    //     if (getcon(&service_manager_context) != 0) {
    //         printf("SELinux: Failed to acquire service_manager context. Aborting.\n");
    //         abort();
    //     }
    // }
